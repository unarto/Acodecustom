...
function FileBrowserInclude(mode, info, doesOpenLast = true) {
	...
	return new Promise((resolve, reject) => {
		...
		const $fbMenu = Contextmenu({
			innerHTML: () => {
				const fbSettings = appSettings.value.fileBrowser;
				const showHiddenText = fbSettings.showHiddenFiles ? "✓ " + (strings["hide hidden files"] || "Hide Hidden Files") : (strings["show hidden files"] || "Show Hidden Files");
				
				const sortBy = fbSettings.sortBy || "name";
				const sortOrder = fbSettings.sortOrder || "asc";

				const nameCheck = sortBy === "name" ? "✓ " : "";
				const sizeCheck = sortBy === "size" ? "✓ " : "";
				const modifiedCheck = sortBy === "modified" ? "✓ " : "";
				const extensionCheck = sortBy === "extension" ? "✓ " : "";

				const ascCheck = sortOrder === "asc" ? "✓ " : "";
				const descCheck = sortOrder === "desc" ? "✓ " : "";

				return `
        <li action="toggle-hidden-files">${showHiddenText}</li>
        <hr style="margin: 4px 0; border: none; border-top: 1px solid var(--border-color, #ccc);" />
        <li action="sort-by-name">${nameCheck}${strings["sort by name"] || "Sort by Name"}</li>
        <li action="sort-by-size">${sizeCheck}${strings["sort by size"] || "Sort by Size"}</li>
        <li action="sort-by-modified">${modifiedCheck}${strings["sort by modified"] || "Sort by Date Modified"}</li>
        <li action="sort-by-extension">${extensionCheck}${strings["sort by extension"] || "Sort by Extension"}</li>
        <hr style="margin: 4px 0; border: none; border-top: 1px solid var(--border-color, #ccc);" />
        <li action="sort-order-asc">${ascCheck}${strings["sort ascending"] || "Ascending (A-Z)"}</li>
        <li action="sort-order-desc">${descCheck}${strings["sort descending"] || "Descending (Z-A)"}</li>
        <hr style="margin: 4px 0; border: none; border-top: 1px solid var(--border-color, #ccc);" />
        ...
        <li action="cut">${strings.cut.capitalize(0)}</li>
		...
		async function handleDrop(e) {
			...
			const draggedEl = $content.querySelector(".dragging");
			if (draggedEl) draggedEl.classList.remove("dragging");

			if (!$el || $el.getAttribute("type") !== "folder" || !draggedItemUrl) return;

			const targetDirUrl = $el.querySelector("data-url")?.textContent;
			if (!targetDirUrl || targetDirUrl === currentDir.url || draggedItemUrl === targetDirUrl) return;

			try {
				const fs = fsOperation(draggedItemUrl);
				await fs.moveTo(targetDirUrl);
				toast(strings.success || "Moved successfully");
				reload();
			} catch (err) {
				helpers.error(err);
			}
		...
		$fbMenu.onclick = function (e) {
			...
			if (action === "toggle-hidden-files") {
				appSettings.value.fileBrowser.showHiddenFiles = !appSettings.value.fileBrowser.showHiddenFiles;
				appSettings.update();
				reload();
				return;
			}
			...
			if (action === "sort-by-name") {
				appSettings.value.fileBrowser.sortBy = "name";
				appSettings.update();
				reload();
				return;
			}

			if (action === "sort-by-size") {
				appSettings.value.fileBrowser.sortBy = "size";
				appSettings.update();
				reload();
				return;
			}

			if (action === "sort-by-modified") {
				appSettings.value.fileBrowser.sortBy = "modified";
				appSettings.update();
				reload();
				return;
			}

			if (action === "sort-by-extension") {
				appSettings.value.fileBrowser.sortBy = "extension";
				appSettings.update();
				reload();
				return;
			}
			...
			if (action === "sort-order-asc") {
				appSettings.value.fileBrowser.sortOrder = "asc";
				appSettings.update();
				reload();
				return;
			}

			if (action === "sort-order-desc") {
				appSettings.value.fileBrowser.sortOrder = "desc";
				appSettings.update();
				reload();
				return;
			}
		...
		$addMenu.onclick = async (e) => {
			...
			switch (action) {
				...
				case "addOnedrive": {
					...
					alert(strings.info || "Info", `Integration with ${provider} will require an OAuth flow which opens an external browser. Once authenticated, a new remote file system wrapper will be mounted here. (Integration placeholder)`);
					// Mock adding the storage for demonstration
					const dummyId = "cloud-" + Date.now();
					storageList.push({
						url: `${provider.toLowerCase()}://${dummyId}/`,
						name: `${provider} Storage`,
						uuid: dummyId,
						storageType: provider.toLowerCase()
					});
		...
		$search.onclick = function () {
			...
			if ($list) {
				searchBar($list, (hide) => (hideSearchBar = hide), undefined, (val) => {
					const children = Array.from($list.children).filter(el => el.classList.contains("tile"));
					const query = val.toLowerCase().trim();
					
					return children.filter((child) => {
						const name = child.getAttribute("name") || child.querySelector(".text span")?.textContent || "";
						const lowerName = name.toLowerCase();
						
						// Filter by specific extension (e.g., ext:js or .js)
						if (query.startsWith("ext:")) {
							const ext = query.replace("ext:", "");
							return lowerName.endsWith("." + ext);
						}
						
						if (query.startsWith(".") && query.length > 1) {
							return lowerName.endsWith(query);
						}
						
						// Filter by type
						if (query === "type:image") {
							return /\.(png|jpg|jpeg|gif|webp|svg|bmp|ico)$/i.test(lowerName);
						}
						if (query === "type:video") {
							return /\.(mp4|mkv|avi|webm)$/i.test(lowerName);
						}
						if (query === "type:audio") {
							return /\.(mp3|wav|ogg|m4a)$/i.test(lowerName);
						}
						if (query === "type:web") {
							return /\.(html|css|js|ts|jsx|tsx|json|xml|php)$/i.test(lowerName);
						}
		...
		async function pasteCopiedItems() {
			...
			const msgTpl = isCut ? (strings["moving items"] || "Moving {count} items...") : (strings["copying items"] || "Copying {count} items...");
			const loadingDialog = loader.create(
				strings.loading,
				msgTpl.replace("{count}", copiedItems.length),
			);
			...
			try {
				for (const url of copiedItems) {
					...
					if (stat.isDirectory && isInsideDirectory(url, targetDirUrl)) {
						alert(
							strings.warning,
							(isCut ? strings["cannot move folder into itself"] : strings["cannot paste folder into itself"]) ||
								(isCut ? "Cannot move a folder into itself" : "Cannot paste a folder into itself"),
						);
						continue;
					}
			...
			async function contextMenuHandler() {
				...
				if (helpers.isFile(type)) {
					...
					if (name.toLowerCase().endsWith(".zip")) {
						options.push(["extract", strings.extract || "Extract", "folder_zip"]);
					}
				} else {
					// Is a directory
					const isBookmarked = storageList.some(s => s.url === url && s.uuid.startsWith('bookmark-'));
					if (isBookmarked) {
						options.push(["unbookmark", strings["remove bookmark"] || "Remove Bookmark", "bookmark"]);
					} else {
						options.push(["bookmark", strings.bookmark || "Bookmark", "bookmark_border"]);
					}
				}
				...
				switch (option) {
					...
					case "extract": {
						const loadingLoader = loader.create(
							...
							strings["extracting"] || "Extracting...",
							{ timeout: 10000 }
						);
						try {
							const zipContent = await fsOperation(url).readFile();
					...
					case "bookmark": {
						const bookmarkName = await prompt(strings["bookmark name"] || "Bookmark Name", name, "text");
						if (!bookmarkName) break;
						storageList.push({
							url,
							name: bookmarkName,
							uuid: "bookmark-" + helpers.uuid(),
							storageType: "bookmark"
						});
						localStorage.storageList = JSON.stringify(storageList);
						toast(strings.success || "Bookmarked successfully");
						break;
					}

					case "unbookmark": {
						const index = storageList.findIndex(s => s.url === url && s.uuid.startsWith('bookmark-'));
						if (index !== -1) {
							storageList.splice(index, 1);
							localStorage.storageList = JSON.stringify(storageList);
							toast(strings.success || "Bookmark removed");
						}
						break;
					}
		...
		async function getDir(url, name) {
			...
			if (url in cachedDir) {
				...
				if (url === "/") {
					...
					try {
						list = (await fs.lsDir()) ?? [];
						const { sortBy } = appSettings.value.fileBrowser;
						if (sortBy === "size" || sortBy === "modified") {
							await Promise.all(
								list.map(async (item) => {
									try {
										const stats = await fsOperation(item.url).stat();
										item.length = stats.length || 0;
										item.lastModified = stats.lastModified || 0;
									} catch (e) {
										item.length = 0;
										item.lastModified = 0;
									}
